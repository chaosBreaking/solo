...
export const hash = (data, options?: IHashOptions) => {
    ...
    if (options?.salt && typeof options.salt === 'string') { data = data + hash(options.salt); }
    const algorithm = HASHER_LIST.indexOf(options.algorithm) >= 0 ? options.algorithm : DEFAULT_ALGORITHM; // 默认为 sha256.
    const inputEncoding = options.inputEncoding || DEFAULT_INPUT_ENCODING; // 'utf8', 'ascii' or 'latin1' for string data, default to utf8 if not specified; ignored for Buffer, TypedArray, or DataView.
    const outputEncoding = options.outputEncoding || 'hex'; // options.output: 留空=》默认输出hex格式；或者手动指定 'buf', hex', 'latin1' or 'base64'
